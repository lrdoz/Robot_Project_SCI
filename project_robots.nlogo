...
to go2
  move-randomly robots
  move-gradient repulsors
  propagate
  move-gradient followers
  tick
end
...
to move-randomly [ agents ]
  ask agents
    [ let p patch-ahead 1
      ifelse (not [no-wall? and (not any? turtles-here) ] of p)
        [ change-heading-and-move ]
        [ ifelse (random 100 < 5)
           [ change-heading-and-move ]
           [ move-to p ]
...
to move-gradient [ agents ]
  ask agents
    [ let v (voisins with [no-wall? and (not any? turtles-here)])
      if (any? v)
        [ move-to min-one-of v [dist + repulsion]]
    ]
end
